<template>
  <view class="shell" :class="`theme-${seasonTheme.key}`" :style="seasonTheme.style">
    <view ref="appScreenRef" class="app-screen">
      <view v-if="detailSlug" ref="detailPageRef" class="detail-page">
        <text v-if="detailLoading" class="empty">正在加载详情...</text>
        <text v-else-if="detailError" class="empty error-state">{{ detailError }}</text>
        <template v-else-if="detailItem">
          <view class="detail-page-head">
            <button @click="closeDetail">
              <Icon icon="solar:alt-arrow-right-linear" />
            </button>
            <text>{{ categoryName(detailItem.category) }} · {{ subCategoryName(detailItem.subCategory) }}</text>
            <text></text>
          </view>

          <view class="detail-hero" :class="{ 'missing-photo': !detailItem.hasRealImage }">
            <image
             ...
             mode="aspectFill" />
            <text>{{ detailItem.detailImageLabel }}</text>
          </view>

          <view class="detail-copy detail-page-copy" :class="`detail-copy--${detailItem.category}`">
            <text>{{ detailItem.englishName }}</text>
            <text>{{ detailItem.name }}</text>
            <view class="detail-tags">
              <text v-for="tag in detailItem.tags" :key="tag">{{ tag }}</text>
            </view>

            <view class="detail-facts">
              <view>
                <text>{{ detailItem.season }}</text>
                <text>成熟期</text>
              </view>
              <view>
                <text>{{ detailItem.storageDays }}天</text>
                <text>建议保存</text>
              </view>
              <view>
                <text>{{ detailItem.bestNow ? '正佳' : '可尝鲜' }}</text>
                <text>当前状态</text>
              </view>
            </view>
            ...
            <view class="detail-block">
              <text>产地与时令</text>
              <text>{{ detailItem.regionNotes }}</text>
              <view v-if="detailItem.regionalSeasons?.length" class="season-region-list">
                <view
                  v-for="season in detailItem.regionalSeasons"
                  :key="season.region"
                  :class="{ active: season.region === activeRegionName }"
                >
                  <text>{{ season.region }}</text>
                  <text>{{ formatMonths(season.months) }}</text>
                  <text>{{ season.note }}</text>
                </view>
              </view>
              <view v-if="detailItem.seasonTerms?.length" class="term-chips">
                <text v-for="term in detailItem.seasonTerms" :key="term">{{ term }}</text>
              </view>
            </view>
            ...
            <view class="detail-block">
              ...
              <view class="nutrition-grid">
                <view v-for="row in nutritionRows(detailItem)" :key="row.label">
                  <text>{{ row.value }}</text>
                  <text>{{ row.label }}</text>
                </view>
              </view>
            ...
            <view v-if="detailItem.varieties?.length" class="detail-block">
              <text>常见品种</text>
              <view class="variety-list variety-photo-list">
                <view v-for="variety in detailItem.varieties" :key="variety.name">
                  <image
                    :src="variety.image"
                    :alt="variety.name"
                    :class="{ 'fruit-outline-image': !variety.hasRealImage && detailItem.category === 'fruit', 'ai-preview-image': !variety.hasRealImage }"
                   mode="aspectFill" />
                  <text v-if="!variety.hasRealImage" class="variety-image-badge">AI生成图</text>
                  <view>
                    <view>
                      <text>{{ variety.name }}</text>
                      <text>{{ variety.season }}</text>
                    </view>
                    <text>{{ variety.note }}</text>
                    <text>{{ variety.origin }}</text>
                    <view>
                      <text v-for="trait in variety.traits" :key="trait">{{ trait }}</text>
                    </view>
                  </view>
                </view>
              </view>
            ...
            <view class="detail-block">
              <text>赏味建议</text>
              <text>{{ detailItem.bestUse }}</text>
            </view>

            <view class="detail-block">
              <text>挑选要点</text>
              <text>{{ detailItem.selectionTips }}</text>
            </view>
          </view>
        </template>
      </view>
      ...
      <scroll-view
        ...
        class="scroll-page"
        scroll-y
        :scroll-top="scrollPageTargetTop"
        @scroll="handleScrollPage"
      >
        <view v-if="showsSearch" class="top-filter-stack" :class="{ 'with-category-tabs': activeTab === 'season' }">
          ...
          <view v-if="activeTab === 'season'" class="category-tabs">
            <button
        ...
        <view v-if="activeTab === 'season'" class="quick-filters">
          <button class="select-pill" @click="showLocation = true">
            <Icon icon="solar:map-point-linear" />
            {{ activeLocation }}
            <Icon icon="solar:alt-arrow-down-linear" />
          </button>
          <button class="select-pill" @click="showMonth = true">
        ...
        >
          <view class="season-badge">
            ...
            <text class="visually-hidden">当前季节</text>
          </view>
          <view class="season-copy">
            <text>{{ seasonName }}</text>
          </view>
          <text>
            <template v-if="seasonBestCount">
              <text class="season-summary-number">{{ seasonBestCount }}</text>
              {{ seasonSummaryText }}
            </template>
          ...
          <view class="season-art" :class="`season-art--${seasonTheme.art}`">
            <text class="season-art-main"></text>
            <text class="season-art-dot one"></text>
            <text class="season-art-dot two"></text>
          </view>
          <view class="season-wave"></view>
        </view>

        <view v-if="activeTab === 'category'" class="category-page">
          <text v-if="loading" class="empty">正在加载分类...</text>
          <text v-else-if="error" class="empty error-state">{{ error }}</text>
          <view v-else class="category-browser">
            <view class="category-sidebar">
              <view class="category-kind-switch">
                <button
              ...
              <view v-if="activeSidebarSection" ref="categorySidebarRef" :key="activeSidebarSection.value" class="category-sidebar-list">
                <button
            ...
            <view ref="categoryContentRef" class="category-content">
              <view class="category-content-title" :class="`category-content-title--${activeTypeGroupMeta.category || 'produce'}`">
                <view>
                  <Icon :icon="activeTypeGroupMeta.icon" />
                  <text>{{ activeTypeGroupMeta.label }}</text>
                </view>
                <text>{{ activeTypeGroupItems.length }}</text>
              </view>
              ...
              <view class="category-list">
                ...
                >
                  ...
                  <text>
                    <text>{{ item.name }}</text>
                    <text class="category-item-meta">
                      <text>{{ item.season }}</text>
                      <text>{{ item.reason }}</text>
                    </text>
                  </text>
                  <text v-if="item.inSeason">当季</text>
                </button>
              </view>
            </view>
          </view>
        ...
        <view v-else-if="activeTab === 'map'" class="origin-map-page">
          <view class="page-title-line">
            <view>
              <Icon icon="inseason:origin-map" />
              <text>产地地图</text>
            </view>
            <view class="province-selector" :class="{ open: showProvincePicker }">
              <button
              ...
              <view v-if="showProvincePicker" class="province-selector__menu">
                <view class="province-selector__search">
                  <Icon icon="solar:magnifer-linear" />
                  <input
                ...
                <view class="province-selector__options">
                  <button
                  ...
                  >
                    <text>{{ profile.province }}</text>
                    <text>{{ profile.count || (profile.fruits.length + profile.vegetables.length) }} 种</text>
                  </button>
                  <text v-if="!filteredProvinceOptions.length">没有匹配省份</text>
                </view>
              </view>
            </view>
          </view>
          ...
          <view class="map-panel">
            <view ref="originMapRef" class="china-map"></view>
            <view class="map-tools">
              <button @click="resetOriginMap">
                <Icon icon="solar:map-point-linear" />
                全国
              </button>
              <text>拖动地图 · 双指缩放</text>
          ...
          <view class="province-produce-card">
            <view>
              <view>
                <text>{{ selectedProvinceProfile.province }}</text>
                <text>{{ selectedProvinceProfile.count }} 种代表蔬果</text>
              </view>
              <text>{{ selectedProvinceProfile.note }}</text>
            </view>
            <view class="province-produce-columns">
              <view class="province-produce-columns__group--fruit">
                <text>
              ...
              <view class="province-produce-columns__group--vegetable">
                <text>
        ...
        <view v-else-if="activeTab === 'ranking'" class="ranking-page">
          <view class="page-title-line">
            <view>
              <Icon icon="solar:ranking-bold" />
              <text>营养成份排行</text>
            </view>
            <text>{{ activeNutrientMeta.unit }}</text>
          </view>

          <view class="ranking-category-toggle">
            <button
          ...
          <view class="ranking-sections">
            <view :class="`ranking-sections__panel--${activeRankingCategory}`">
              <view>
                <view>
                  <Icon :icon="activeRankingCategoryMeta.icon" />
                  <text class="visually-hidden">{{ activeRankingCategoryMeta.label }}</text>
                </view>
                <text>Top {{ activeRankingItems.length }}</text>
              </view>
              <view class="ranking-list">
                <button
                  ...
                  @click="openDetail(item)"
                >
                  <text :class="`ranking-list__rank--${index + 1}`">
                  ...
                  <text>
                    <text>{{ item.name }}</text>
                    <text>{{ rankingCategoryLabel(item) }}</text>
                  </text>
                  <text>{{ nutrientValue(item) }}</text>
                </button>
              </view>
            </view>
          </view>
        ...
        <view v-else class="produce-section">
          <text v-if="loading" class="empty">正在加载当季食材...</text>
          <text v-else-if="error" class="empty error-state">{{ error }}</text>

          <view v-else-if="showsSeparatedProduce" class="split-produce">
            <view
            ...
            >
              <view class="split-title">
                ...
                <text>{{ group.inSeasonCount }}/{{ group.items.length }}</text>
              </view>
              <view class="produce-grid">
                <ProduceCard
          ...
          <view v-else class="produce-grid">
            <ProduceCard
          ...
          <text v-if="!loading && !error && !visibleProduce.length" class="empty">{{ emptyText }}</text>
        </view>
...
const iconGlyphs = {
  ...
  'solar:apple-bold': '果',
  'solar:home-2-bold': '当',
  'solar:widget-5-bold': '类',
  'solar:map-bold': '地',
  'solar:ranking-bold': '榜',
  'solar:bolt-bold': '闪',
  'solar:chart-bold': '图',
  'solar:check-circle-bold': '✓',
  'solar:circle-linear': '○',
  'inseason:greens': '菜',
  'inseason:produce-all': '全',
  'inseason:season-basket': '篮',
  'inseason:origin-map': '图',
  'inseason:rank-champion': '冠',
  'inseason:rank-runner-up': '亚',
  'inseason:rank-third': '季',
  'inseason:nutrient-vitamin-c': 'C',
  'inseason:nutrient-fiber': '纤',
  'inseason:nutrient-protein': '蛋',
  'inseason:nutrient-potassium': '钾',
  'inseason:nutrient-calorie': '卡'
}

const Icon = defineComponent({
  props: ['icon'],
  setup(props, { attrs }) {
    return () => h('text', { ...attrs, class: ['icon-text', attrs.class] }, iconGlyphs[props.icon] || '•')
  }
})
...
const scrollPageTargetTop = ref(0)
...
const seasonThemes = {
  1: { key: 'winter', art: 'snowdrop', primary: '#86a8cf', accent: '#d8e8fb', blush: '#fff3f0', ink: '#38607d', glow: 'rgba(216, 232, 251, 0.72)', bg: '/static/assets/season-bg/01-xiaohan.svg' },
  2: { key: 'spring', art: 'peach', primary: '#8fcf9a', accent: '#f8c8d8', blush: '#f7f4c8', ink: '#497f52', glow: 'rgba(248, 200, 216, 0.55)', bg: '/static/assets/season-bg/02-lichun.svg' },
  3: { key: 'spring', art: 'sprout', primary: '#8fcf9a', accent: '#f7b7c8', blush: '#fff2b8', ink: '#4c8456', glow: 'rgba(247, 183, 200, 0.58)', bg: '/static/assets/season-bg/03-jingzhe.svg' },
  4: { key: 'spring', art: 'rain', primary: '#86cfa6', accent: '#ffd3a8', blush: '#f4f6bf', ink: '#3f7e5e', glow: 'rgba(255, 211, 168, 0.58)', bg: '/static/assets/season-bg/04-qingming.svg' },
  5: { key: 'summer', art: 'lotus', primary: '#88c9bd', accent: '#ffd58e', blush: '#dff5f0', ink: '#3a7b70', glow: 'rgba(255, 213, 142, 0.5)', bg: '/static/assets/season-bg/05-lixia.svg' },
  6: { key: 'summer', art: 'grain', primary: '#6fbea8', accent: '#ffc98f', blush: '#e4f6d9', ink: '#2f7566', glow: 'rgba(111, 190, 168, 0.42)', bg: '/static/assets/season-bg/06-mangzhong.svg' },
  7: { key: 'summer', art: 'melon', primary: '#7fbfd8', accent: '#ffe08a', blush: '#e4f7f6', ink: '#347489', glow: 'rgba(127, 191, 216, 0.42)', bg: '/static/assets/season-bg/07-xiaoshu.svg' },
  8: { key: 'autumn', art: 'leaf', primary: '#d4a76f', accent: '#ffc6a6', blush: '#f8eec7', ink: '#8a6131', glow: 'rgba(255, 198, 166, 0.56)', bg: '/static/assets/season-bg/08-liqiu.svg' },
  9: { key: 'autumn', art: 'pear', primary: '#c99a74', accent: '#f4c7a1', blush: '#f5edc9', ink: '#7e5a39', glow: 'rgba(244, 199, 161, 0.5)', bg: '/static/assets/season-bg/09-bailu.svg' },
  10: { key: 'autumn', art: 'persimmon', primary: '#c58f7e', accent: '#ffd3ad', blush: '#f6e5c8', ink: '#7b5547', glow: 'rgba(255, 211, 173, 0.52)', bg: '/static/assets/season-bg/10-hanlu.svg' },
  11: { key: 'winter', art: 'frost', primary: '#94abc8', accent: '#d7e7f4', blush: '#f8eadf', ink: '#526d8b', glow: 'rgba(215, 231, 244, 0.66)', bg: '/static/assets/season-bg/11-lidong.svg' },
  12: { key: 'winter', art: 'snow', primary: '#8da7c7', accent: '#dceafb', blush: '#f7eee6', ink: '#4f6988', glow: 'rgba(220, 234, 251, 0.68)', bg: '/static/assets/season-bg/12-daxue.svg' }
}
...
function scrollToTop() {
  scrollPageTargetTop.value = scrollPageTop.value
  nextTick(() => {
    scrollPageTargetTop.value = 0
    scrollPageTop.value = 0
  })
}
...
async function loadDetail(slug, fallback = null) {
  ...
  try {
    const { data, statusCode } = await requestApi(`/api/produce/${slug}`)
    if (statusCode < 200 || statusCode >= 300) throw new Error('detail api failed')
...
const ProduceCard = defineComponent({
  ...
  setup(props, { emit }) {
    return () => h('view', {
      ...
      h('view', { class: 'card-copy' }, [
        h('text', props.item.name),
        h('text', props.item.season),
...
const ChoiceSheet = defineComponent({
  ...
  emits: ['close', 'select'],
  setup(props, { emit }) {
    return () => h('view', { class: 'sheet-backdrop', onClick: event => emit('close') }, [
      h('view', { class: 'choice-sheet' }, [
        h('view', { class: 'choice-title' }, [
          h('text', props.title),
          h('button', { type: 'button', onClick: () => emit('close') }, [h(Icon, { icon: 'solar:close-circle-bold' })])
        ]),
        h('view', { class: 'choice-list' }, props.options.map(option => h('button', {
            key: option,
            type: 'button',
